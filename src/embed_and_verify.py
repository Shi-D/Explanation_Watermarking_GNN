...
from backdoor_based_watermarking import *
...
def accuracy(output, labels, verbose=False):
    ...
    # pred = output.argmax(dim=1)  # Using argmax directly
...
#     elif augment_subgraphs_separately==False:
...
def setup_loss_dict():
    loss_dict = {'loss_primary':torch.tensor(0.0),   'loss_watermark': torch.tensor(0.0),
                 'loss_primary_weighted':torch.tensor(0.0),  'loss_watermark_weighted':torch.tensor(0.0), 
                 }
    return loss_dict
...
def continue_interrupted_training(dataset_name, arch, num_subgraphs, seed, numLayers=None, hDim=None, dropout=None, train_ratio=0.6, previous_starting_epoch=0):
    ...
    if numLayers is not None:
        config.node_classifier_kwargs['numLayers']=numLayers
    if hDim is not None:
...
class Trainer():
    def __init__(self, data, dataset_name, target_number_matches=None):
        ...
        self.use_batching = config.node_classifier_kwargs['use_batching']
        self.batch_size = config.node_classifier_kwargs['batch_size']
        validate_kwargs()
        ...
        self.clf_only = self.optimization_kwargs['clf_only']

        if self.clf_only==True:
            train_nodes_mask = get_classification_train_nodes(self.data, None, None, self.data.x.shape[0], train_with_test_set=False, clf_only=True)
            self.train_node_indices = train_nodes_mask.nonzero(as_tuple=True)[0]
        ...
        if config.using_our_method==False:
            ...
            if self.clf_only==False:
                self.train_nodes_to_consider_mask = get_classification_train_nodes(self.data, self.all_subgraph_indices, self.sacrifice_method, self.data.x.shape[0], train_with_test_set=False, clf_only=False)
            ...
            if config.watermark_random_backdoor==True:
                ...
                self.graphlime_backdoored_data  = get_subgraph_from_node_indices(self.data, self.graphlime_backdoored_node_indices)
        ...
        if config.preserve_edges_between_subsets==False:
            ...
            print(f"Train - Nodes: {len(self.train_node_indices)}, Max edge index: {self.edge_index_train_unaugmented.max().item()}")
        ...
        self.test_node_indices = self.test_mask.nonzero(as_tuple=True)[0]
        if config.preserve_edges_between_subsets==False:
            print('B')
            ...
            print(f"Test - Nodes: {len(self.test_node_indices)}, Max edge index: {self.edge_index_test.max().item()}")
        ...
        self.val_node_indices = self.val_mask.nonzero(as_tuple=True)[0]
        if config.preserve_edges_between_subsets==False:
            print('C')
            ...
            print(f"Val - Nodes: {len(self.val_node_indices)}, Max edge index: {self.edge_index_val.max().item()}")
        ...
        self.unaugmented_data = self.data.clone()
        ...
        # extract train/val/test data as their own graphs (for disconnected edges forward pass -- not preferred, but still an option)
        self.train_data_unaugmented = get_subgraph_from_node_indices(self.unaugmented_data, self.train_node_indices)
        self.val_data_unaugmented = get_subgraph_from_node_indices(self.unaugmented_data, self.val_node_indices)
        self.test_data_unaugmented = get_subgraph_from_node_indices(self.unaugmented_data, self.test_node_indices)

        self.loader_registry = DataLoaderRegistry()
    ...
    def print_epoch_status_(self):
        print_epoch_status(self.epoch, self.loss_primary_weighted, self.acc_trn, self.acc_val, self.acc_test, config.using_our_method, 
                            ...
                            clf_only=self.clf_only, acc_trigger=self.acc_trigger, acc_graphlime_backdoor=self.acc_graphlime_backdoor)


    def augment_data_per_epoch(self, augment_seed):
        if self.node_aug is not None or self.edge_aug is not None:
            augment_seed=update_seed(augment_seed)
            if self.clf_only==True:
                self.augmented_data = augment_data(self.data, self.clf_only,self.node_aug, self.edge_aug, self.train_node_indices, self.all_subgraph_indices, seed=augment_seed)
            else:
                self.augmented_data = augment_data(self.data, self.clf_only,self.node_aug, self.edge_aug, self.train_nodes_to_consider, self.all_subgraph_indices, seed=augment_seed)
        else:
            self.augmented_data = self.data
        # extract train/val/test data as their own graphs (for disconnected edges forward pass -- not preferred, but still an option)
        self.train_data_augmented = get_subgraph_from_node_indices(self.augmented_data, self.train_node_indices)
        self.val_data_augmented = get_subgraph_from_node_indices(self.augmented_data, self.val_node_indices)
        self.test_data_augmented = get_subgraph_from_node_indices(self.augmented_data, self.test_node_indices)
    ...
    def choose_closure_method(self):
        if config.using_our_method==False:
            if self.clf_only==True:
    ...
    def update_history_one_epoch_(self):
    ...
    def train(self, debug_multiple_subgraphs=True, save=True, print_every=10, continuation=False, starting_epoch=0):
        ...
        print("data.y 0s, 1s:", torch.sum(self.data.y==0), torch.sum(self.data.y==1))
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            self.epoch=epoch
            self.augment_data_per_epoch(augment_seed)

            closure = self.choose_closure_method()

            closure()

            if self.use_batching==False:
                self.optimizer.step()


            self.update_history_one_epoch_()
            if self.epoch%print_every==0:

                self.print_epoch_status_()                
    ...
    def forward(self, x, edge_index, dropout, mode='train', mask=None, data=None):
        # subgraphs have their own forward pass function so this is only for classification training data
        ...
        if self.use_batching==True:
            # Initialize loader registry if it doesn't exist
            if not hasattr(self, 'loader_registry'):
                self.loader_registry = DataLoaderRegistry()
            loader = self.loader_registry.get_loader(data, mask, self.batch_size, mode)
            log_logits = self.batched_forward(data, dropout, mode, mask, loader)
        else:
            log_logits = self.node_classifier(x, edge_index, dropout)
        return log_logits
    

    def batched_forward(self, data, dropout, mode, mask, loader):
        original_training = self.node_classifier.training

        log_logits = torch.zeros(data.x.size(0), self.node_classifier.outDim if self.node_classifier.skip_connections else self.node_classifier.outDim, dtype=data.x.dtype, device=data.x.device)

        # If loader was provided but might have been reset due to unpickling
        if loader is not None and not hasattr(loader, '_dataset'):
            # Create a new loader if the provided one is invalid
            kwargs = {'batch_size': self.batch_size, 'num_workers': 6, 'persistent_workers': True}
            if mode == 'train':
                print('train')
                # [25, 10]: 25 neighbors for the first 'hop' and 10 for the second 'hop
                loader = NeighborLoader(data, input_nodes=mask, num_neighbors=[25, 10], shuffle=True, **kwargs)
            else:
                # [-1]: unbounded number of neighbors
        ...
        with torch.set_grad_enabled(original_training):
            ...
            for count, batch in enumerate(loader):
                ...
                dropout = 0.5 if mode=='train' else 0
                output = self.node_classifier(batch.x, batch.edge_index, dropout=dropout)

                if mode=='train':
                    y = batch.y[:batch.batch_size]
                    y_hat = output[:batch.batch_size]
                    loss = F.nll_loss(y_hat, y)
                    loss.backward()
                    self.optimizer.step()

                acc = accuracy(y_hat, y, verbose=False)
        ...
        self.node_classifier.train(original_training)
        return log_logits
    ...
    def closure_primary(self, use_batching=False):
        # optimize
        mode, dropout ='train',config.node_classifier_kwargs['dropout']
        self.optimizer.zero_grad()
        print('Forward train')
        if config.preserve_edges_between_subsets==True:
            # forward pass on entire graph (here represented as 'augmented_data')
            data = self.augmented_data
            log_logits = self.forward(data.x, data.edge_index, dropout, mode, data.train_mask, data) 
            log_logits_train = log_logits[self.train_node_indices]#log_logits[self.train_nodes_to_consider]

        elif config.preserve_edges_between_subsets==False:
            data = self.train_data_augmented
            log_logits_train    = self.forward(data.x, data.edge_index, dropout, mode, torch.ones_like(self.data.x), self.data)
        ...
        if self.epoch%10==0:
            print('Forward eval')
            if config.preserve_edges_between_subsets==True:
                # Use 'self.train_node_indices' instead of 'self.train_nodes_to_consider' -- the latter only applies when watermarking
                data, masks = self.unaugmented_data, [self.train_node_indices, self.val_mask, self.test_mask]
                if self.use_batching==True:
                    log_logits_train_eval, log_logits_val, log_logits_test = [self.forward(data.x, data.edge_index,dropout,mode,mask,data)[mask] for mask in masks]
                else:
                    log_logits_inference = self.forward(data.x, data.edge_index, dropout, mode, None, data)
                    log_logits_train_eval, log_logits_val, log_logits_test = [log_logits_inference[mask] for mask in masks]

            elif config.preserve_edges_between_subsets==False:
                # forward pass only on subset nodes / edges
                train_mask_, val_mask_, test_mask_ = [torch.ones_like(self.x_train_unaugmented), torch.ones_like(self.x_val), torch.ones_like(self.x_test)]
                log_logits_train_eval = self.forward(self.x_train_unaugmented,  self.edge_index_train_unaugmented,  dropout, mode, train_mask_, self.train_data_unaugmented)
                log_logits_val        = self.forward(self.x_val,                self.edge_index_val,                dropout, mode, val_mask_,   self.val_data_unaugmented)
                log_logits_test       = self.forward(self.x_test,               self.edge_index_test,               dropout, mode, test_mask_,  self.test_data_unaugmented)
    ...
    def closure_watermark(self):
        ...
        mode, dropout ='train',config.node_classifier_kwargs['dropout']
        if config.preserve_edges_between_subsets==True:
            data, mask = self.augmented_data, self.train_nodes_to_consider
            log_logits_all = self.forward(data.x, data.edge_index, dropout, mode, mask, data) # will apply mask if self.use_batching==True
            log_logits_train = log_logits_all[self.train_nodes_to_consider] # regardless of whether we apply the mask above (in the use_batching==True case), the outputs will still reflect ALL nodes, not just training.
        
        elif config.preserve_edges_between_subsets==False:
            data, mask = self.train_data_augmented, torch.ones_like(self.train_nodes_to_consider)
            log_logits_train = self.forward(data.x, data.edge_index, dropout, mode, mask, data) # will apply mask if self.use_batching==True
        ...
        # eval
        dropout, mode, data, masks = 0, 'eval', self.unaugmented_data, [self.train_nodes_to_consider, self.val_mask, self.test_mask]
        if config.preserve_edges_between_subsets==True:
            if self.use_batching==True:
                log_logits_train_eval, log_logits_val, log_logits_test = [self.forward(data.x, data.edge_index, dropout, node, mask, data)[mask] for mask in masks]
            # regardless of whether we apply the mask above (in the use_batching==True case), the outputs will still reflect ALL nodes, not just training.
            else:
                # forward pass on entire graph (here represented as 'unaugmented_data' -- dont augment graph for inference, training only)
                log_logits_inference = self.forward(data.x, data.edge_index, dropout, mode, None, data)  # don't need to pass mask argumnt, because batched_forward() won't be called
                log_logits_train_eval, log_logits_val, log_logits_test = [log_logits_inference[mask] for mask in masks]
                log_logits_train_eval_option_2 = log_logits_inference[self.train_nodes_to_consider_mask]
                assert torch.eq(log_logits_train_eval, log_logits_train_eval_option_2).all()
        elif config.preserve_edges_between_subsets==False:
            # forward pass only on subset nodes / edges
            train_data_, val_data_, test_data = self.train_data_unaugmented, self.val_data_unaugmented, self.test_data_unaugmented
            train_mask, val_mask, test_mask = [torch.ones_like(train_data_.x), torch.ones_like(val_data_.x), torch.ones_like(test_data.x)]    
            log_logits_train_eval = self.forward(train_data_.x,    self.edge_index_train,    dropout, mode, train_mask,  train_data_)
            log_logits_val        = self.forward(val_data_.x,      self.edge_index_val,      dropout, mode, val_mask,    val_data_)
            log_logits_test       = self.forward(test_data.x,     self.edge_index_test,     dropout, mode, test_mask,   test_data)
        ...
        if self.optimization_kwargs['clf_only']==False:
            ...
            self.weighted_losses_backward = self.weighted_losses
    ...
    def closure_random_backdoor_watermark(self):
        ...
        # optimize
        if config.preserve_edges_between_subsets==True:
            # mask below only applied if self.use_batching==True
            log_logits    = self.forward(self.augmented_data.x, 
                                         self.augmented_data.edge_index, 
                                         dropout=config.node_classifier_kwargs['dropout'], 
                                         mode='train', 
                                         mask=self.train_nodes_to_consider, 
                                         #  data_type='aug'
                                         data = self.augmented_data
                                         )
            ...
            # mask below only applied if self.use_batching==True
            log_logits_train    = self.forward(self.x_train, 
                                               self.edge_index_train, 
                                               dropout=config.node_classifier_kwargs['dropout'], 
                                               mode='train', 
                                               mask= torch.ones(self.data.x_train.shape()), 
                                               # data_type='aug'
                                               data = self.train_data_augmented
                                               )

        log_logits_trigger = self.forward(self.trigger_graph.x, 
                                          self.trigger_graph.edge_index, 
                                          dropout=0, 
                                          mode='train', 
                                          mask=torch.ones(self.trigger_graph.x.shape()), 
                                          # data_type='unaug'
                                          data = self.trigger_graph
                                          )
        ...
        if config.preserve_edges_between_subsets==True:
            # forward pass on entire graph (here represented as 'unaugmented_data' -- dont augment graph for inference, training only)
            if self.use_batching==True:
                log_logits_train_eval = self.forward(self.unaugmented_x, 
                                                     self.unaugmented_edge_index, 
                                                     dropout=0, 
                                                     mode='eval', 
                                                     mask=self.train_nodes_to_consider,
                                                     data = self.unaugmented_data
                                                     )
                log_logits_val = self.forward(self.unaugmented_x, 
                                              self.unaugmented_edge_index, 
                                              dropout=0, 
                                              mode='eval', 
                                              mask=self.val_mask,
                ...
                log_logits_test = self.forward(self.unaugmented_x, 
                                              self.unaugmented_edge_index, 
                                              dropout=0, 
                                              mode='eval', 
                                              mask=self.test_mask,
                ...
                log_logits_inference = self.forward(self.unaugmented_x, 
                                                    self.unaugmented_edge_index, 
                                                    dropout=config.node_classifier_kwargs['dropout'], 
                                                    mode='eval',
                                                    data = self.unaugmented_data # technically can be anything because batch_forward() will not be called.
                                                    )  # don't need to pass mask argumnt, because batched_forward() won't be called. 
                log_logits_train_eval = log_logits_inference[self.train_node_indices]
                log_logits_val = log_logits_inference[self.val_node_indices]
                log_logits_test = log_logits_inference[self.test_node_indices]
        elif config.preserve_edges_between_subsets==False:
            # forward pass on entire graph (here represented as 'unaugmented_data' -- dont augment graph for inference, training only)
            log_logits_train_eval = self.forward(self.x_train_unaugmented, 
                                                 self.edge_index_train_unaugmented, 
                                                 dropout=0, 
                                                 mode='eval', 
                                                 mask = torch.ones_like(self.train_nodes_to_consider),
                                                 data = self.train_data_unaugmented
                                                 )
                                                 
            log_logits_val        = self.forward(self.x_val, 
                                                 self.edge_index_val, 
                                                 dropout=0, 
                                                 mode='eval', 
                                                 mask = torch.ones_like(self.val_mask),
                                                 data = self.val_data_unaugmented
                                                 )
            log_logits_test       = self.forward(self.x_test, 
                                                 self.edge_index_test, 
                                                 dropout=0, 
                                                 mode='eval', 
                                                 mask = torch.ones_like(self.test_mask),
                                                 data = self.test_data_unaugmented
                                                 )

        log_logits_trigger_eval = self.forward(self.trigger_graph.x, 
                                               self.trigger_graph.edge_index, 
                                               dropout=0, 
                                               mode='eval', 
                                               mask=torch.ones(self.trigger_graph.x.shape()),
    ...
    def closure_graphlime_backdoor_watermark(self):
        ...
        if config.preserve_edges_between_subsets==True:
            # mask below only applied if self.use_batching==True
            log_logits_train = self.forward(self.augmented_data.x, 
                                               self.augmented_data.edge_index, 
                                               dropout=config.node_classifier_kwargs['dropout'], 
                                               mode='train', 
                                               mask=self.train_nodes_to_consider,
        ...
        if config.preserve_edges_between_subsets==True:
            ...
            if self.use_batching==True:
                ...
                log_logits_val = log_logits_inference[self.val_node_indices]
                log_logits_test = log_logits_inference[self.test_node_indices]
                log_logits_backdoor = log_logits_inference[self.graphlime_backdoored_node_indices]
            ...
            log_logits_backdoor   = self.forward(self.x_graphlime_backdoor,
                                                 self.edge_index_graphlime_backdoor, 
                                                 dropout=0, 
                                                 mode='eval',
                                                 mask = torch.ones_like(self.x_graphlime_backdoor), 
                                                 data=self.graphlime_backdoored_data
                                                 )
        self.acc_trn  = accuracy(log_logits_train_eval, self.y_train_unaugmented,verbose=False)
        self.acc_val  = accuracy(log_logits_val, self.y_val,verbose=False)
        self.acc_test = accuracy(log_logits_test, self.y_test,verbose=False)
...
def compute_likelihood_of_matches(betas_wmk__, sample_mean_matches, sample_std_matches, verbose=False):
    # I dont think this is called by anything, unless a notebook somewhere?
