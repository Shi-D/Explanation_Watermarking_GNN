...
class CreateMaskTransform:
    def __init__(self, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2, seed=0, random=False):
    ...
    def __call__(self, data):
        ...
        # NOTE: this is BROKEN! this should ALWAYS be random. Should re-collect results for all datasets except Reddit2 (which I did not process with this function.)
        try:
            if self.random==True:
                # i.e., keep this
            ...
            ## have this exception built in because previous datasets were transformed with a version of this class that did not have the "random" argument.
            ## So for those datsets, it will throw the error "AttributeError: 'CreateMaskTransform' object has no attribute 'random".
            ## Ideally, re-run all experiments with the new version of this class.
            indices = torch.tensor(range(num_nodes))
